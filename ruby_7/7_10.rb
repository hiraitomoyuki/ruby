...
# ===
# ===はおもにcase文のwhen節で使われる。例えば、case文では正規表現を使って次のような条件がかける
text = '03-1234-5678'
...
# このコードを実行すると内部的には
/^\d{3}-\d{4}$/ === text
/^\d{3}-\/\d{1,2}\/\d{1,2}$/ === text

# のように、"when節のオブジェクト === case節のオブジェクト"の結果を評価している。(左辺はwhen節のオブジェクト)
# StringクラスやArrayクラスもClassクラスのインスタンスであり、Classクラスは===を再定義しているので、次のようなcase文を書いてオブジェクトのデータ型(所属するクラス)を識別することができる
value = [1, 2, 3]
